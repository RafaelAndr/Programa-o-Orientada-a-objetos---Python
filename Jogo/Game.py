...
class Game:
	...
	def __createRooms(self):
		...
		auditorium = Room("in the auditorium")
		...
		lab.setExits(outside, office, None, None, auditorium, None)
		office.setExits(None, None, None, lab, None, None)
		auditorium.setExits(None, None, None, None, None, lab)
	...
	def __goRoom(self, command):
		...
		nextRoom = None
		if (direction == "north"):
			nextRoom = self.__currentRoom.getExit(direction)
		if (direction == "east"):
			nextRoom = self.__currentRoom.getExit(direction)
		if (direction == "west"):
			nextRoom = self.__currentRoom.getExit(direction)
	...
	def __printLocationInfo(self):
		...
		if(self.__currentRoom.getExit('north') != None):
			exits += "north "
		if(self.__currentRoom.getExit('east') != None):
			exits += "east "
		if(self.__currentRoom.getExit('south') != None):
			exits += "south "
		if(self.__currentRoom.getExit('west') != None):
			exits += "west "
		if(self.__currentRoom.getExit('up') != None):
			exits += "up "
		if(self.__currentRoom.getExit('down') != None):
			exits += "down "
